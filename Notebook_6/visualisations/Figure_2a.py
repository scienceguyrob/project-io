"""
Figure 2a — Interactive Gaussian Probability Density Function Explorer (Not-Spam Class)
=====================================================================
...
def show():
    ...
    mu_init    = 310.0    # mean email length for not-spam (words)
    sigma_init = 40.0     # spread of email lengths for not-spam (words)
    x1_init    = 42.0    # the query email from the worked example (words)

    # ── Build figure and axes once ───────────────────────────────────────────
    fig, ax = plt.subplots(num="Notebook6 Figure 2a", figsize=(10, 6))
    ...
    x1_line = ax.axvline(x1_init, color='darkorange', lw=2.0,
                         ...
                         label=r'$x_1$ (query email length — not-spam class)')
    ...
    def on_reset(_):
        # Return to the not-spam class defaults from the worked example.
